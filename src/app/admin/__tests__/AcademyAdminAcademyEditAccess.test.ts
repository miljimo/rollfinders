...
test("academy admin academy detail route renders the edit form with platform controls disabled", () => {
  const pageSource = readFileSync("src/app/admin/academies/[id]/page.tsx", "utf8");
  const formSource = readFileSync("src/app/admin/academies/AcademyForm.tsx", "utf8");
...
test("academy admin academy update preserves platform-only academy fields server-side", () => {
  const actionSource = readFileSync("src/app/admin/academies/actions.ts", "utf8");
  const apiSource = readFileSync("src/app/api/admin/academies/[id]/route.ts", "utf8");
