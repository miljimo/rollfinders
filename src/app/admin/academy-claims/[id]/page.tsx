  ...
  return (
    <PageShell>
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <Link href="/admin/academy-claims" className="inline-flex items-center gap-2 text-sm font-bold text-teal-800">
          <ArrowLeft size={16} aria-hidden />
          Academy Claims
        </Link>

        {!result.ok ? (
          <ErrorState message={result.message} status={result.status} />
        ) : (
          <ClaimDetail claim={result.data} decision={decision} error={error} />
        )}
      </section>
    </PageShell>
  );
...
function ClaimDetail({ claim, decision, error }: { claim: AcademyClaimDetail; decision?: string; error?: string }) {
...
function ErrorState({ status, message }: { status: number; message: string }) {
  ...
  return (
    <div className="mt-6 rounded-lg border border-stone-200 bg-white px-5 py-12 text-center shadow-sm">
      ...
      <Button href="/admin/academy-claims" variant="secondary" className="mt-5">Back to Claims</Button>
    </div>
  );
}
