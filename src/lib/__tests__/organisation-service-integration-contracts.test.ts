...
const organisationClient = readFileSync(resolve(root, "src/lib/organisation-service.ts"), "utf8");
...
const serverRoutes = readFileSync(resolve(root, "services/organisation/internal/server/server.go"), "utf8");
...
describe("Organisation service RollFinders integration", () => {
  it("configures RollFinders to call Organisation Service as the organisation data source", () => {
    const apiCompose = readFileSync(resolve(root, "services/api/compose.yml"), "utf8");
