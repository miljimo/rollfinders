...
describe("analytics feature contracts", () => {
  ...
  it("accepts valid public analytics events and rejects invalid payloads through a generic route response", () => {
    const source = readSource("src/app/api/analytics/events/route.ts");
  ...
  it("keeps founder analytics API read-only and Super Admin-only", () => {
    const source = readSource("src/app/api/admin/analytics/route.ts");
  ...
  it("exposes founder analytics dashboard UI only to Super Admins", () => {
    ...
    const superAdminOnlySource = source.match(/\.\.\.\(superAdmin[\s\S]*href:\s*"\/dashboard\?panel=analytics"[\s\S]*:\s*\[\]\)/)?.[0] ?? "";
  ...
  it("records search demand only from explicit search form submissions", () => {
    ...
    const locationSearchSource = readSource("src/components/LocationSearchForm.tsx");
    const openMatFilterSource = readSource("src/components/OpenMatLocationFilterForm.tsx");
