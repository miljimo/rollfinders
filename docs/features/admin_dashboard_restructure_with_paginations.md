...
# Admin Dashboard Restructure With Pagination

## Objective

Restructure the RollFinders admin dashboard so operational data is easier to scan and does not load unbounded record lists. The dashboard should provide a compact overview, separate management sections, and independent pagination for each dataset.

## Scope

In scope:

* Admin dashboard layout restructure
* Summary metrics for major operational entities
* Server-side pagination for dashboard lists
* Pagination for academies, open mats, users, outbound emails, and invalid emails
* Preservation of existing super-admin actions
...
* Bulk operations
* New user roles
* Dedicated detail dashboards

## Requirements

### Dashboard Overview

The admin dashboard must show high-level counts for:
...
The page must separate operational areas into clear panels:
...
Each paginated section must:

* Load a fixed page size
* Show the visible item range and total count
* Provide Previous and Next controls
* Preserve other active pagination query parameters
...
Existing authorization rules must remain intact:

* Platform admins can access the dashboard.
* Super admins can create users and manage invalid email records.
* Protected super-admin users cannot be deleted or disabled.

## Acceptance Criteria

1. Admin dashboard displays summary metrics.
2. Each major data group is in a clearly labelled section.
3. Lists are paginated independently.
4. Pagination works through shareable URL query parameters.
5. Existing admin actions continue to work.
