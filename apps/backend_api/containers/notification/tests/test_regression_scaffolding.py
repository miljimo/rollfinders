...
BACKEND_ROOT = NOTIFICATION_ROOT.parents[1]
REPO_ROOT = BACKEND_ROOT.parents[1]
NOTIFICATION_DOCS_ROOT = REPO_ROOT / "docs" / "services" / "notification"
...
class NotificationRegressionScaffoldingTests(unittest.TestCase):
    def test_ticket_010_runbook_covers_required_regression_gates(self) -> None:
        runbook = NOTIFICATION_DOCS_ROOT / "ticket010-regression-runbook.md"
    ...
    def test_notification_table_ownership_stays_in_notification_migrations(self) -> None:
        migration_files = [
            path
            for service_root in (BACKEND_ROOT / "migrations").iterdir()
            if service_root.is_dir()
            for path in sql_files(service_root)
        ]
    ...
    def test_prd_and_ticket_define_tables_before_executable_db_tests_land(self) -> None:
        source_docs = [
            NOTIFICATION_DOCS_ROOT / "product.md",
            NOTIFICATION_DOCS_ROOT
            / "Feature"
            / "Tickets"
            / "Refinement"
    ...
    def test_retry_schedule_contract_is_pinned(self) -> None:
        product = read_text(NOTIFICATION_DOCS_ROOT / "product.md")
        ticket = read_text(
            NOTIFICATION_DOCS_ROOT
            / "Feature"
            / "Tickets"
