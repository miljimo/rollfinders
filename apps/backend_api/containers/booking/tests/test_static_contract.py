...
BACKEND_ROOT = BOOKING_ROOT.parents[1]
REPO_ROOT = BACKEND_ROOT.parents[1]
BOOKING_COMMAND_ROOT = BACKEND_ROOT / "cmd" / "services" / "booking"
BOOKING_SERVICE_ROOT = BACKEND_ROOT / "internal" / "services" / "booking"
BOOKING_MIGRATIONS_ROOT = BACKEND_ROOT / "migrations" / "booking"
BOOKING_DOCS_ROOT = REPO_ROOT / "docs" / "services" / "booking"
...
class BookingSkeletonContractTests(unittest.TestCase):
    def test_required_service_skeleton_artifacts_exist(self) -> None:
        expected_paths = [
            BACKEND_ROOT / "go.mod",
            BOOKING_COMMAND_ROOT / "api",
            BOOKING_SERVICE_ROOT / "config",
            BOOKING_SERVICE_ROOT / "handlers",
            BOOKING_SERVICE_ROOT / "server",
            BOOKING_SERVICE_ROOT / "dataaccess",
            BOOKING_SERVICE_ROOT / "databases",
            BOOKING_SERVICE_ROOT / "environments",
            BOOKING_ROOT / "Dockerfile",
            BOOKING_ROOT / ".dockerignore",
            BOOKING_ROOT / "compose.yml",
        ]
    ...
    def test_health_and_ready_routes_are_declared(self) -> None:
        files = all_text_files(BOOKING_SERVICE_ROOT)
        self.assertTrue(files, "expected booking service package files to exist")
...
class BookingOpenApiContractTests(unittest.TestCase):
    def openapi_files(self) -> list[Path]:
        candidates = [
            BOOKING_DOCS_ROOT / "api",
            BOOKING_DOCS_ROOT / "openapi",
            BOOKING_DOCS_ROOT,
        ]
        files: list[Path] = []
...
class BookingMigrationContractTests(unittest.TestCase):
    def migration_root(self) -> Path:
        return BOOKING_MIGRATIONS_ROOT
    ...
    def test_duplicate_active_registered_booking_is_protected(self) -> None:
        ...
        self.assertRegex(migration_sql, r"where[\s\S]+status[\s\S]+(pending|confirmed)")
    ...
    def test_function_and_procedure_files_use_camel_case_names(self) -> None:
        ...
        for path in routine_files:
            ...
            if not is_camel_case(stem):
                bad_files.append(path.relative_to(BACKEND_ROOT).as_posix())
