...
func migratePermissions(ctx context.Context, usersDB *sql.DB, tx *sql.Tx) (int, error) {
	...
	for rows.Next() {
		...
		resourceID := gatewayroutes.StableResourceID(key)
		_, err := tx.ExecContext(ctx, `
...
func migrateUserRoles(ctx context.Context, usersDB *sql.DB, tx *sql.Tx) (int, error) {
	...
	for rows.Next() {
		...
		_, err := tx.ExecContext(ctx, `
				...
				ON CONFLICT (user_id, role_id, COALESCE(organisation_id, ''), COALESCE(application_id, ''), COALESCE(resource_id, ''))
...
func migrateUserPermissions(ctx context.Context, usersDB *sql.DB, tx *sql.Tx) (int, error) {
	...
	for rows.Next() {
		...
		_, err := tx.ExecContext(ctx, `
				...
				ON CONFLICT (user_id, permission_id, COALESCE(organisation_id, ''), COALESCE(application_id, ''), COALESCE(resource_id, ''))
