...
func TestErrorWithStatusUsesSharedStatusError(t *testing.T) {
	rec := httptest.NewRecorder()

	ErrorWithStatus(rec, NewStatusError(http.StatusForbidden, "not_authorised", "Not authorised.", nil, map[string]string{"permission": "academy.read"}), http.StatusInternalServerError)
	...
	if payload["error"] != "Not authorised." {
		t.Fatalf("unexpected error: %#v", payload["error"])
	}
	if payload["error_code"] != "not_authorised" {
		t.Fatalf("unexpected error code: %#v", payload["error_code"])
	}
	if payload["success"] != false {
		t.Fatalf("unexpected success flag: %#v", payload["success"])
	}
}
