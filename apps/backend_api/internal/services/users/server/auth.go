...
func (s *server) sessions(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.db.Function(r.Context(), "users.sessions_list", userID, limit, offset)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "Unable to list sessions.")
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"sessions": rows, "pagination": pagination(limit, offset, len(rows))})
}

func (s *server) revokeSession(w http.ResponseWriter, r *http.Request) {
	sessionID := handlers.Param(r, string(gatewayroutes.ParamSessionId))
