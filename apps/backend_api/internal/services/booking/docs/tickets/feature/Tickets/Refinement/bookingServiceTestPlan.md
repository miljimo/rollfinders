...
- `apps/backend_api/internal/services/booking/docs/prds/proposal.md`
...
```sh
python -m unittest discover apps/backend_api/containers/booking/tests
```
...
- Go tests should run with `cd apps/backend_api/containers/booking && go test ./...` once `go.mod` exists.
