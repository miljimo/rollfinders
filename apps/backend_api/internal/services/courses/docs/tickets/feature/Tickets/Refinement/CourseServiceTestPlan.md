...
* Migration entry points follow the service include style with the Courses camelCase file name `apps/backend_api/internal/services/courses/migrations/001_coreSchema.sql`: schema include first, `SET search_path`, ordered `\ir` includes for schema/types/tables/functions/procedures, then idempotent `INSERT INTO schema_migrations(version) ... ON CONFLICT DO NOTHING`.
...
### `apps/backend_api/internal/services/courses/migrations/migration_contract_test.go`
