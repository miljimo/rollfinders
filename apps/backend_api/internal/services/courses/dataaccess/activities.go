...
func ListActivities(ctx context.Context, db databases.DataContext, courseID string, limit int, offset int) ([]Activity, error) {
	rows, err := db.Function(ctx, `courses."courseActivitiesList"`, courseID, limit, offset)
	if err != nil {
		return nil, err
	}
