...
func (s *server) ListPermissionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	out, err := s.repo.listPermissions(r.Context(), limit, offset)
	if err != nil {
		s.logUnexpectedRepoError(r, err)
		handlers.ErrorWithStatus(w, repoStatusError(err), http.StatusInternalServerError)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"permissions": out, "pagination": handlers.Pagination(limit, offset, len(out))})
}
