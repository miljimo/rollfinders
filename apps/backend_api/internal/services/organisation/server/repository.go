...
func (r *repository) listOrganisations(ctx context.Context, limit int, offset int) ([]Organisation, error) {
...
func (r *repository) listApplications(ctx context.Context, limit int, offset int) ([]Application, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, organisation_id, name, slug, status, created_at, updated_at
		FROM applications
		ORDER BY name ASC
...
func (r *repository) listApplicationServices(ctx context.Context, applicationID string, limit int, offset int) ([]ApplicationService, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT application_id, service_key, enabled, created_at, updated_at
		FROM application_services
		WHERE application_id = $1
		ORDER BY service_key ASC
		LIMIT LEAST(GREATEST(COALESCE($2, 10), 1), 100)
