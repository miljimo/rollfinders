package endpoints

import (
	"net/http"

	"rollfinders/internal/services/academy/dataaccess"
)

func (s *server) listAcademyMembers(w http.ResponseWriter, r *http.Request) {
	academyID := cleanString(r.PathValue("academy_id"))
	if academyID == "" {
		writeError(w, r, http.StatusBadRequest, "invalid_request", "academy_id is required.", nil)
		return
	}
	db, ok := s.withDataContext(w, r)
	if !ok {
		return
	}
	defer db.Close()
	limit := intQuery(r.URL.Query().Get("limit"), 10)
	offset := intQuery(r.URL.Query().Get("offset"), 0)
	members, err := dataaccess.ListAcademyMembers(r.Context(), db, academyID, limit, offset)
	if err != nil {
		s.writeDataError(w, r, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"members": members, "pagination": pagination(limit, offset, len(members))})
}
