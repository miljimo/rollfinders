...
import { RegisterAcademySelector } from "./RegisterAcademySelector";
...
export default async function RegisterPage({
  ...
  const academyOptions = academySelectOptions(
    await listAcademiesFromAcademyService({ limit: 100 }),
  );
      ...
      <section className="mx-auto grid w-full max-w-5xl gap-6 px-4 py-7 sm:px-6 md:py-10 lg:grid-cols-[minmax(0,1fr)_22rem] lg:py-12">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-stone-950">
            Join your academy on RollFinders
          </h1>
          <p className="mt-3 max-w-2xl text-stone-700">
            Create your own account and connect it to your academy if an admin
            has not created one for you yet.
          </p>

          {error ? (
            <p
              role="alert"
              className="mt-5 rounded-md border border-red-200 bg-red-50 p-3 text-sm font-semibold leading-6 break-words text-red-800"
            >
              {error}
            </p>
          ) : null}
          ...
          <form
            action={registerPractitioner}
            className="mt-5 grid gap-4 rounded-lg border border-stone-200 bg-white p-4 shadow-sm sm:p-5"
          >
            <RegisterAcademySelector
              academy={academy}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1.5 text-sm font-semibold text-stone-800">
                ...
                <input
                  name="firstName"
                  required
                  className="min-h-12 rounded-md border border-stone-300 bg-stone-50 px-3 text-base font-normal text-stone-950 focus:border-teal-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-700/20"
                />
              </label>
              <label className="grid gap-1.5 text-sm font-semibold text-stone-800">
                Last name
...
function academySelectOptions(
  ...
  return academies.map((academy) => ({
    ...
    meta: [academy.slug, academy.city, academy.postcode]
      .filter(Boolean)
      .join(" "),
  }));
}
