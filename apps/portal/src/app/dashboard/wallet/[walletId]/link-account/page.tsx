...
export default async function LinkExternalWalletAccountPage({
  ...
  if (!isPlatformAdminRole(user.role) && !isAcademyAdminRole(user.role))
    redirect("/dashboard");
  ...
  const linkedAccounts =
    wallet.walletType === "external"
      ? await listLinkedWalletAccounts(
          wallet.id,
          user.accessToken,
          user.id,
        ).catch(() => [])
  ...
  const walletDetailsHref = `/dashboard/wallet?walletDialog=wallet-details&walletId=${encodeURIComponent(wallet.id)}`;
  if (linkedAccount && linkedAccount.status !== "PENDING") {
    redirect(walletDetailsHref);
  }
  const ownerAccountName = await resolveWalletOwnerAccountName(
    wallet.ownerId,
    user,
  );
    ...
    <main className="min-h-screen bg-stone-50 px-4 py-6 text-slate-950 md:px-8">
      <div className="mx-auto grid max-w-5xl gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Button href={walletDetailsHref} variant="secondary">
        ...
        <header>
          ...
          <p className="mt-2 max-w-2xl text-sm font-semibold text-slate-600">
            Choose which provider account should be linked to this external
            wallet.
          </p>
        </header>
        ...
        {wallet.walletType !== "external" ? (
          <Notice
            tone="warning"
            text="Only external wallets can be linked to provider accounts."
          />
        ) : linkedAccount?.provider === "STRIPE" &&
          linkedAccount.status === "PENDING" ? (
          <StripePendingLinkPanel
          ...
          <Notice
            tone="success"
            text="This external wallet already has a linked account."
          />
        ) : (
...
function WalletSummary({
  ...
  const rows = [
    ...
    { label: "Status", value: <TableStatusBadge status={wallet.status} /> },
    {
      ...
      value: linkedAccount
        ? linkedAccount.displayName || linkedAccount.provider
        : "Not linked",
    },
  ];
    ...
    <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      ...
      <dl className="grid gap-3">
        {rows.map((row) => (
          <div
            key={row.label}
            className="grid gap-1 border-b border-stone-100 pb-3 last:border-b-0 last:pb-0 md:grid-cols-[10rem_minmax(0,1fr)]"
          >
            <dt className="text-sm font-black text-stone-600">{row.label}</dt>
            <dd className="break-all text-sm font-semibold text-slate-950">
              {row.value}
            </dd>
          </div>
        ))}
...
function StripePendingLinkPanel({
  currency,
  linkedAccount,
  walletId,
}: {
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-amber-900">
            Continue Stripe Connect to finish the provider requirements. The
            external wallet will remain inactive until Stripe verifies the
            account for charges and payouts.
          </p>
          <p className="mt-2 break-all text-xs font-semibold text-amber-800">
            Provider account:{" "}
            {linkedAccount.providerAccountId ||
              linkedAccount.externalReference ||
              "Not available"}
          </p>
        </div>
...
async function resolveWalletOwnerAccountName(
  ...
  return (
    owner?.name ??
    owner?.email ??
    (ownerId === currentUser.id ? currentUser.email : undefined) ??
    ownerId
  );
}
...
function Notice({ text, tone }: { text: string; tone: "success" | "warning" }) {
  ...
  return (
    <div
      className={`rounded-lg border px-4 py-4 text-sm font-bold ${className}`}
    >
      {text}
    </div>
  );
