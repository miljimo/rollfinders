...
export default async function LoginPage({
  ...
  searchParams: Promise<{ callbackUrl?: string | string[]; redirect?: string | string[]; registered?: string | string[]; email?: string | string[]; warning?: string | string[]; verifyEmail?: string | string[] }>;
}) {
  ...
  const verifyEmail = Array.isArray(params.verifyEmail) ? params.verifyEmail[0] : params.verifyEmail;
  return (
    <PageShell>
      <section className="mx-auto w-full max-w-md px-4 py-7 sm:px-6 md:py-10 lg:py-12">
        ...
        {registered === "1" ? (
          <p className="mt-4 rounded-md border border-teal-200 bg-teal-50 p-3 text-sm font-semibold leading-6 text-teal-900">
            Account created{email ? ` for ${email}` : ""}. {verifyEmail === "1" ? "Check your inbox and verify your email before signing in." : warning === "academy-link" ? "Sign in and ask your academy admin to connect your account." : "Sign in to continue."}
          </p>
        ) : null}
