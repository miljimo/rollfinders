...
function scopeMatches(
  assignmentScope: Scope | undefined,
  requestedScope: Scope,
): boolean {
  if (!assignmentScope) return true;

  return (Object.entries(assignmentScope) as [keyof Scope, string][]).every(
    ([key, value]) => requestedScope[key] === value,
  );
}
...
function authorize(
  ...
  const directDeny = model.userPermissions.some(
    (assignment) =>
  ...
  if (directDeny)
    return { authorized: false, decision: "deny", reason: "direct_deny" };

  const directAllow = model.userPermissions.some(
    (assignment) =>
      assignment.userId === subjectId &&
      assignment.permission === permission &&
...
function translateLegacyUsersServiceData(
  legacy: LegacyUsersServiceData,
): AuthorisationModel {
  ...
  for (const mapping of legacy.rolePrivileges) {
    rolePermissions.set(mapping.roleKey, [
      ...new Set([
        ...(rolePermissions.get(mapping.roleKey) ?? []),
        mapping.privilegeKey,
      ]),
    ]);
  }

  return {
    permissions: legacy.privileges.map((privilege) => ({
      code: privilege.key,
    })),
    ...
    userRoles: legacy.userRoles.map((assignment) => ({
      ...
      scope: assignment.organisationId
...
describe("Authorisation Service decision contract", () => {
  const model: AuthorisationModel = {
    ...
    roles: [{ key: "ACADEMY_ADMIN", permissions: ["academy.update"] }],
    userRoles: [
      { userId: "role-user", roleKey: "ACADEMY_ADMIN", scope: academyScope },
    ],
    userPermissions: [
      {
        userId: "direct-allow-user",
      ...
      },
      {
        ...
        effect: "ALLOW",
        scope: { ...academyScope, resourceId: "academy-2" },
      },
    ],
  };
  ...
  it("allows a role permission in matching scope", () => {
    assert.deepEqual(
      authorize(model, "role-user", "academy.update", academyScope),
      {
        authorized: true,
        decision: "allow",
      },
  ...
  it("allows a direct user permission assignment", () => {
    assert.deepEqual(
      authorize(model, "direct-allow-user", "booking.cancel", academyScope),
      {
        authorized: true,
        decision: "allow",
      },
  ...
  it("denies when a direct user deny conflicts with role or direct allow", () => {
    assert.deepEqual(
      authorize(model, "denied-user", "academy.update", academyScope),
      {
        authorized: false,
        decision: "deny",
        reason: "direct_deny",
  ...
  it("denies when the requested scope does not match the assignment scope", () => {
    assert.deepEqual(
      authorize(model, "scoped-user", "payment.refund", academyScope),
      {
        authorized: false,
        decision: "deny",
  ...
  it("fails closed for unknown permissions", () => {
    assert.deepEqual(
      authorize(model, "role-user", "academy.delete", academyScope),
      {
        authorized: false,
        decision: "deny",
        reason: "unknown_permission",
...
describe("RollFinders authorisation client super admin contract", () => {
  it("keeps super admins as all-privilege users before service decisions", () => {
    ...
    const authorizeFunction =
      source.match(/export async function authorize[\s\S]*?\n}\n/)?.[0] ?? "";
  ...
  it("uses the direct authorisation service URL for server-side permission checks when configured", () => {
    ...
    assert.match(source, /AUTHORISATION_PUBLIC_BASE_URL/);
    assert.match(source, /normalizeBaseUrl\(directBaseUrl\) \+ "\/v1"/);
    assert.match(source, /apiGatewayPath\("\/v1\/authorisation"\)/);
  });

  it("keeps academy wallet permissions aligned in compatibility fallback", () => {
    const source = readSource("apps/portal/src/lib/authorisation-service.ts");
    const fallbackFunction =
      source.match(/function legacyPermissionFallback[\s\S]*?\n}\n/)?.[0] ?? "";
...
describe("Users Service migration verification contract", () => {
  const legacy: LegacyUsersServiceData = {
    ...
    userPermissions: [
      {
        userId: "standard-user",
        privilegeKey: "academy.update",
        effect: "ALLOW",
        organisationId: "org-1",
      },
      {
        userId: "blocked-admin",
        privilegeKey: "academy.update",
        effect: "DENY",
        organisationId: "org-1",
      },
  ...
  it("translates privileges into permissions and preserves row-count equivalence", () => {
    ...
    assert.equal(
      translated.roles.flatMap((role) => role.permissions).length,
      legacy.rolePrivileges.length,
    );
    assert.equal(translated.userRoles.length, legacy.userRoles.length);
    assert.equal(
      translated.userPermissions.length,
      legacy.userPermissions.length,
    );
  });
  ...
  it("preserves representative legacy access outcomes after translation", () => {
    ...
    assert.equal(
      authorize(translated, "platform-admin", "academy.update", academyScope)
        .authorized,
      true,
    );
    assert.equal(
      authorize(translated, "standard-user", "academy.update", academyScope)
        .authorized,
      true,
    );
    assert.equal(
      authorize(translated, "standard-user", "users.admin.access", academyScope)
        .authorized,
      false,
    );
    assert.equal(
      authorize(translated, "blocked-admin", "academy.update", academyScope)
        .authorized,
      false,
    );
  });
