...
describe("practitioner academy registration contracts", () => {
  it("creates public practitioner accounts through the users service and then links academy membership", () => {
    ...
    const userServiceSource = readSource(
      "apps/portal/src/lib/users-service.ts",
    );
    ...
    assert.match(
      actionSource,
      /if \(!academyId\)[\s\S]*failureRedirect[\s\S]*registerUserAccount/,
    );
  ...
  it("requires public users to verify email before login", () => {
    const loginSource = readSource(
      "apps/backend_api/internal/services/users/server/server.go",
    );
    const runtimeMutations = readSource(
      "apps/backend_api/internal/services/users/migrations/procedures/008_runtimeMutations.sql",
    );
  ...
  it("allows admin users to verify pending user emails", () => {
    const userActionsSource = readSource(
      "apps/portal/src/app/admin/users/actions.ts",
    );
    const usersServiceSource = readSource(
      "apps/portal/src/lib/users-service.ts",
    );
  ...
  it("exposes a public register page with academy selection and locked academy entry points", () => {
    ...
    const registerAcademySelectorSource = readSource(
      "apps/portal/src/app/register/RegisterAcademySelector.tsx",
    );
    ...
    assert.match(registerPageSource, /RegisterAcademySelector/);
    assert.match(registerAcademySelectorSource, /AutoCompleteTextField/);
    assert.match(registerAcademySelectorSource, /name="academyId"/);
    assert.match(registerAcademySelectorSource, /name="academySlug"/);
    assert.match(registerAcademySelectorSource, /Choose a different academy/);
  ...
  it("uses generic academy membership routes for user profile linking", () => {
    ...
    const profileSource = readSource(
      "apps/portal/src/lib/rollfinder-user-profiles.ts",
    );

    assert.match(academyServiceSource, /request\(\s*`?\/v1\/memberships/);
    ...
    assert.match(
      profileSource,
      /removeAcademyMembershipInAcademyService\(membership\.id/,
    );
  });
