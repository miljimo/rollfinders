...
describe("deployment safety contracts", () => {
  ...
  it("configures the EC2 frontend to use the independently deployed API", () => {
    const deploy = readSource("scripts/cicd/deploy-ec2-app.sh");
    const terraform = readSource("infrastructure/terraform/main.tf");
    const locals = readSource("infrastructure/terraform/locals.tf");
    const dnsModule = readSource("infrastructure/terraform/modules/app_dns_records/main.tf");

    assert.match(terraform, /API_PUBLIC_BASE_URL\s+=\s+local\.api_base_url/);
    assert.match(locals, /"api\.\$\{var\.hosted_zone_name\}"/);
    assert.match(locals, /"\$\{var\.environment_name\}\.api\.\$\{var\.hosted_zone_name\}"/);
    assert.match(deploy, /aws ssm get-parameters-by-path/);
    assert.match(deploy, /env_file: \.env/);
    assert.doesNotMatch(dnsModule, /aws_route53_record" "api"/);
  });
});
