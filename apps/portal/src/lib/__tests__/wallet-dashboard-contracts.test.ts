...
test("wallet service client uses API gateway wallet endpoints", () => {
  ...
  assert.match(
    source,
    /\/v1\/wallets\/\$\{encodeURIComponent\(walletId\)\}\/linked-accounts/,
  );
...
test("wallet dashboard route renders through the app dashboard shell", () => {
  ...
  assert.match(
    dashboard,
    /href:\s*"\/dashboard\/wallet"[\s\S]*label:\s*"Wallet"/,
  );
  ...
  assert.match(
    dashboard,
    /Choose a user you have permission to manage as the wallet owner/,
  );
  ...
  assert.match(
    dashboard,
    /\/dashboard\/wallet\/\$\{encodeURIComponent\(wallet\.id\)\}\/link-account/,
  );
  const linkAccountPage = readSource(
    "apps/portal/src/app/dashboard/wallet/[walletId]/link-account/page.tsx",
  );
  assert.match(linkAccountPage, /Link External Wallet Account/);
  assert.match(linkAccountPage, /ProviderSelectionForm/);
  const providerSelectionForm = readSource(
    "apps/portal/src/app/dashboard/wallet/[walletId]/link-account/ProviderSelectionForm.tsx",
  );
  assert.match(providerSelectionForm, /Stripe Connect/);
  assert.match(providerSelectionForm, /\/api\/wallet\/stripe-connect/);
  assert.match(providerSelectionForm, /name="provider" value="STRIPE"/);
  assert.match(providerSelectionForm, /Continue Stripe Connect/);
  assert.doesNotMatch(providerSelectionForm, /Bank Account/);
  assert.doesNotMatch(providerSelectionForm, /PayPal/);
  assert.doesNotMatch(providerSelectionForm, /id:\s*"BANK"/);
  assert.doesNotMatch(providerSelectionForm, /id:\s*"CARD"/);
  assert.doesNotMatch(providerSelectionForm, /id:\s*"PAYPAL"/);
  const walletStripeConnectRoute = readSource(
    "apps/portal/src/app/api/wallet/stripe-connect/route.ts",
  );
  assert.match(
    walletStripeConnectRoute,
    /\/api\/wallet\/stripe-connect\/refresh/,
  );
  assert.match(walletStripeConnectRoute, /createLinkedWalletAccount/);
  assert.match(walletStripeConnectRoute, /provider !== "STRIPE"/);
  const walletStripeRefreshRoute = readSource(
    "apps/portal/src/app/api/wallet/stripe-connect/refresh/route.ts",
  );
  ...
  assert.match(
    form,
    /destinationWalletOptions[\s\S]*option\.id !== sourceWalletId/,
  );
  ...
  assert.doesNotMatch(
    form,
    /dashboard submit action is not connected to the wallet transfer API/,
  );
...
test("wallet transfer permission is used for authorisation", () => {
  const authorisation = readSource(
    "apps/portal/src/lib/authorisation-service.ts",
  );
...
test("wallet dashboard side panel exposes transaction command only", () => {
  ...
  assert.match(
    dashboard,
    /href === "\/dashboard\/wallet"[\s\S]*children: walletNavigationSections/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=wallets", icon:\s*"wallet", label:\s*"Wallets"/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=transfers", icon:\s*"transfers", label:\s*"Transfers"/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=approvals", icon:\s*"approvals", label:\s*"Approvals"/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=payouts", icon:\s*"payouts", label:\s*"Payouts"/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=reserves", icon:\s*"reserves", label:\s*"Reserves"/,
  );
  assert.doesNotMatch(
    dashboard,
    /href:\s*"\/dashboard\/wallet\?walletView=settings", icon:\s*"settings", label:\s*"Settings"/,
  );
...
test("wallet dashboard shows wallets and transaction panel without aggregate balance cards or extra action UI", () => {
  ...
  assert.match(
    source,
    /href="\/dashboard\/wallet\?walletView=transactions&walletDialog=create-transaction"/,
  );
  ...
  assert.match(
    source,
    /transactions\.slice\(start, start \+ transactionPageSize\)/,
  );
  assert.match(source, /pagination=\{\{/);
  assert.match(
    source,
    /href="\/dashboard\/wallet\?walletDialog=create-wallet"/,
  );
  ...
  assert.match(
    source,
    /selectedWallet\(wallets: WalletRecord\[\], searchParams: WalletDashboardSearchParams\)/,
  );
