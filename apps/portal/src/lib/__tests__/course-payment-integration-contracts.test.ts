...
describe("course payment service integration", () => {
  ...
  it("routes RollFinders frontend service clients through the API gateway", () => {
    ...
    const serviceSources = [
      ...
      readSource("apps/portal/src/app/v1/checkouts/[id]/callbacks/[result]/route.ts"),
    ].join("\n");
  ...
  it("renders checkout controls on course detail through a client handoff", () => {
    ...
    const formSource = readSource("apps/portal/src/app/courses/[id]/CourseCheckoutForm.tsx");
  ...
  it("keeps verified free events bookable without starting checkout", () => {
    const publicDetailSource = readSource("apps/portal/src/components/PublicEventDetailPage/PublicEventDetailPage.tsx");
    const freeButtonSource = readSource("apps/portal/src/components/FreeEventBookingButton/FreeEventBookingButton.tsx");
    const openMatPageSource = readSource("apps/portal/src/app/open-mats/[id]/page.tsx");
    const coursePageSource = readSource("apps/portal/src/app/courses/[id]/page.tsx");
    const actionSource = readSource("apps/portal/src/app/courses/[id]/payment-actions.ts");
  ...
  it("requires a ready academy Stripe Connect account before collecting paid event payments", () => {
    const helperSource = readSource("apps/portal/src/lib/academy-payment-account.ts");
    const coursePageSource = readSource("apps/portal/src/app/courses/[id]/page.tsx");
    const openMatPageSource = readSource("apps/portal/src/app/open-mats/[id]/page.tsx");
    const actionSource = readSource("apps/portal/src/app/courses/[id]/payment-actions.ts");
    ...
    assert.match(helperSource, /error\.status === 401 \|\| error\.status === 403/);
  ...
  it("stores Stripe Connect accounts against academy or platform ownership", () => {
    const stripeConnectSource = readSource("apps/portal/src/lib/stripe-connect.ts");
    const connectRouteSource = readSource("apps/portal/src/app/api/payments/stripe-connect/route.ts");
    const refreshRouteSource = readSource("apps/portal/src/app/api/payments/stripe-connect/refresh/route.ts");
    const disconnectRouteSource = readSource("apps/portal/src/app/api/payments/stripe-connect/disconnect/route.ts");
    const dashboardSource = readSource("apps/portal/src/app/dashboard/AdminDashboardWorkspace.tsx");
    const paymentsClientSource = readSource("apps/portal/src/lib/payments.ts");
    const schemaSource = readSource("prisma/schema.prisma");
    const serviceMigrationSource = readSource("apps/backend_api/internal/services/payments/migrations/tables/013_paymentAccountSettings.sql");
    const serviceEndpointSource = readSource("apps/backend_api/internal/services/payments/server/EndpointStripeConnect.go");
    const servicePersistenceSource = readSource("apps/backend_api/internal/services/payments/server/StorePersistence.go");
    const serviceProviderSource = readSource("apps/backend_api/internal/services/payments/server/provider.go");
  ...
  it("proxies public checkout callbacks to the private payment service", () => {
    const routeSource = readSource("apps/portal/src/app/v1/checkouts/[id]/callbacks/[result]/route.ts");
    const paymentStatusSource = readSource("apps/portal/src/app/payments/status/page.tsx");
    const bookingSource = readSource("apps/portal/src/lib/bookings.ts");
