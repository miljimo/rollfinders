...
const schema = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/schema/001_authorisation_schema.sql",
  "utf8",
);
const permissionsTable = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/tables/001_permissions.sql",
  "utf8",
);
const userRolesTable = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/tables/004_user_roles.sql",
  "utf8",
);
const userPermissionsTable = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/tables/005_user_permissions.sql",
  "utf8",
);
const auditEventsTable = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/tables/006_authorisation_audit_events.sql",
  "utf8",
);
const resourcesTable = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/tables/007_resources.sql",
  "utf8",
);
const resourceScopeMigration = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/009_resource_scope_table.sql",
  "utf8",
);
const removeApplicationServicePermissionsMigration = readFileSync(
  "apps/backend_api/internal/services/authorisation/migrations/011_remove_application_service_permissions.sql",
  "utf8",
);
const seedCatalog = readFileSync(
...
const endpointSources = [
  ...
  .map((file) =>
    readFileSync(
      `apps/backend_api/internal/services/authorisation/server/${file}`,
      "utf8",
    ),
  )
  .join("\n");
...
test("Authorisation migration follows the service migration folder structure", () => {
  ...
  assert.match(
    migration,
    /\\ir\s+procedures\/001_seedAuthorisationCatalog\.sql/,
  );
});
...
test("Authorisation permissions are capability codes without numeric levels", () => {
  ...
  assert.doesNotMatch(
    repository,
    /permissions\.level|p\.level|permission\.Level/,
  );
});
...
test("Authorisation permission definitions can be global or scoped to organisation and application", () => {
  ...
  assert.match(
    permissionsTable,
    /\bresource_id\s+text\s+NOT\s+NULL\s+REFERENCES\s+resources\(id\)/i,
  );
  assert.match(
    permissionsTable,
    /UNIQUE\s+NULLS\s+NOT\s+DISTINCT\s+\(resource_id,\s+organisation_id,\s+application_id\)/i,
  );
  assert.doesNotMatch(
    permissionsTable,
    /\bcode\s+text\s+NOT\s+NULL\s+UNIQUE\b/i,
  );
  assert.match(
    seedCatalog,
  ...
  assert.match(
    repository,
    /SELECT \* FROM permission_create\(\$1, \$2, \$3, \$4, \$5\)/,
  );
  assert.match(
    repository,
    /p\.OrganisationID,\s*p\.ApplicationID,\s*p\.ResourceID/,
  );
  assert.match(
    repository,
    /SELECT \* FROM permission_row_by_code\(\$1, \$2, \$3\)/,
  );
  assert.match(
    repository,
    /code,\s*scope\.OrganisationID,\s*scope\.ApplicationID/,
  );
});

test("Authorisation permission IDs are generated and not semantic permission codes", () => {
  const permissionInsertIndex = seedCatalog.indexOf(
    "INSERT INTO permissions (id, resource_id)",
  );
  const rolesInsertIndex = seedCatalog.indexOf(
    "INSERT INTO roles",
    permissionInsertIndex,
  );
  const permissionInsertBlock = seedCatalog.slice(
    permissionInsertIndex,
    rolesInsertIndex,
  );
  ...
  assert.match(
    permissionInsertBlock,
    /ON CONFLICT ON CONSTRAINT permissions_resource_scope_key/,
  );
...
test("Authorisation scopes use a resource table for resource identifiers", () => {
  assert.match(
    resourcesTable,
    /CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+resources/i,
  );
  ...
  assert.match(
    resourceScopeMigration,
    /FOREIGN\s+KEY\s+\(resource_id\)\s+REFERENCES\s+resources\(id\)/i,
  );
  assert.match(
    repository,
    /SELECT \* FROM resource_upsert\(\$1, \$2, \$3, \$4\)/,
  );
  assert.match(
    repository,
    /resource\.ID,\s*resource\.Name,\s*resource\.Description,\s*resource\.Target/,
  );
  assert.match(
    repository,
    /SELECT \* FROM user_role_assign\(\$1, \$2, \$3, \$4, \$5, \$6, \$7\)/,
  );
  assert.match(
    repository,
    /a\.Scope\.OrganisationID,\s*a\.Scope\.ApplicationID,\s*a\.Scope\.ResourceID/,
  );
  assert.match(
    repository,
    /SELECT \* FROM user_permission_assign\(\$1, \$2, \$3, \$4, \$5, \$6, \$7, \$8\)/,
  );
  ...
  assert.doesNotMatch(
    repository,
    /INSERT\s+INTO\s+user_permissions\s+\([^)]*resource_type[^)]*\)/,
  );
  assert.doesNotMatch(
    repository,
    /INSERT\s+INTO\s+user_roles\s+\([^)]*resource_type[^)]*\)/,
  );
});
...
test("Authorisation does not own application-service enablement", () => {
  ...
  assert.doesNotMatch(
    repository,
    /permission_application_enabled|permissionApplicationEnabled/i,
  );
  assert.match(
    removeApplicationServicePermissionsMigration,
    /DROP TABLE IF EXISTS application_service_permissions/i,
  );
  assert.match(
    removeApplicationServicePermissionsMigration,
    /DROP FUNCTION IF EXISTS permission_application_enabled\(text, text\)/i,
  );
});
...
test("Authorisation seed keeps platform payment revenue separate from academy payment metrics", () => {
  ...
  const academyWhereIndex = seedCatalog.indexOf(
    "WHERE r.key IN ('ACADEMY_OWNER', 'ACADEMY_ADMIN')",
  );
  const academyJoinIndex = seedCatalog.lastIndexOf(
    "JOIN resources resource ON resource.id = p.resource_id AND resource.name IN",
    academyWhereIndex,
  );
  const academyConflictIndex = seedCatalog.indexOf(
    "ON CONFLICT DO NOTHING;",
    academyWhereIndex,
  );
  const academyRoleBlock = seedCatalog.slice(
    academyJoinIndex,
    academyConflictIndex,
  );
  ...
  assert.doesNotMatch(
    academyRoleBlock,
    /payment\.report\.platform_revenue\.read/,
  );
});
...
test("Authorisation seed includes wallet dashboard permissions", () => {
  for (const permission of [
    ...
    assert.ok(
      seedCatalog.includes(`'${permission}'`),
      `${permission} must be seeded`,
    );
  }
});

test("Authorisation seed assigns wallet permissions to academy admin roles", () => {
  const academyWhereIndex = seedCatalog.indexOf(
    "WHERE r.key IN ('ACADEMY_OWNER', 'ACADEMY_ADMIN')",
  );
  const academyJoinIndex = seedCatalog.lastIndexOf(
  ...
  for (const permission of [
    ...
    assert.match(academyRoleBlock, new RegExp(permission.replace(".", "\\.")));
  }
});

test("Authorisation migration reconciles academy wallet permissions for existing databases", () => {
  const migration = readFileSync(
    "apps/backend_api/internal/services/authorisation/migrations/020_seed_academy_wallet_permissions.sql",
    "utf8",
  );
  assert.match(migration, /CALL "seedAuthorisationCatalog"\(\)/);
  assert.match(migration, /r\.key IN \('ACADEMY_OWNER', 'ACADEMY_ADMIN'\)/);
  assert.match(migration, /resource\.name LIKE 'wallet\.%'/);
});
