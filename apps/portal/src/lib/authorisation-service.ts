...
export async function listUserAuthorisationRoles(userId: string, actor?: AuthorisationActor | null) {
  try {
...
export async function listAuthorisationRolePermissions(roleId: string, actor?: AuthorisationActor | null) {
  try {
...
export async function listAuthorisationResources(actor?: AuthorisationActor | null) {
  try {
...
export async function listEffectiveUserPermissions(userId: string, scope: AuthorisationScope = {}, actor?: AuthorisationActor | null) {
  const params = new URLSearchParams();
...
export async function listUserPermissionAssignments(userId: string, actor?: AuthorisationActor | null) {
  try {
...
export async function updateUserAuthorisationPermissions(
  ...
  const remainingEffectiveCodes = new Set(
    revokeCodes.size > 0
      ? (await listEffectiveUserPermissions(userId, assignmentScope, actor)).map((permission) => permission.code)
      : [],
  );
...
export async function replaceUserAuthorisationRole(
  ...
  const managedRoleIds = new Set(roles.filter((role) => managedRoleKeys.has(role.key)).map((role) => role.id));
