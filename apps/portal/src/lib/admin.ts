...
export async function getCurrentUser() {
  ...
  try {
    const account = await getUserAccount(user.id, accessToken);
    const accountUser = { ...account.user, accessToken };
    if (accountUser.academyId) return accountUser;

    const membership = (await listAcademyMembershipsForUserFromAcademyService(accountUser.id, accountUser))[0];

    return { ...accountUser, academyId: membership?.academyId ?? null };
  } catch (error) {
    if (error instanceof UserServiceError && (error.status === 401 || error.status === 403 || error.status === 404)) return null;
    throw error;
  }
}
