...
func Open(ctx context.Context, dbName string, connString string) (DataContext, error) {
	return (&dataConnectionImpl{}).createDataContext(ctx, dbName, connString, nil)
}
...
func (dc *dataContextImpl) Function(ctx context.Context, functionName string, params ...interface{}) (DBResults, error) {
	...
	return dc.Query(ctx, fmt.Sprintf("SELECT * FROM %s%s", functionName, dc.createParameterPlaceholder(params...)), params...)
}
...
func (dc *dataContextImpl) Procedure(ctx context.Context, procedureName string, params ...interface{}) (RowsAffected, error) {
	...
	return dc.Execute(ctx, fmt.Sprintf("CALL %s%s", procedureName, dc.createParameterPlaceholder(params...)), params...)
}
