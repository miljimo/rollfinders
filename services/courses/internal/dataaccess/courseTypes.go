...
func ListCourseTypes(ctx context.Context, db databases.DataContext, organisationID string) ([]CourseType, error) {
	rows, err := db.Function(ctx, `courses."courseTypesList"`, organisationID)
	if err != nil {
		return nil, err
	}
