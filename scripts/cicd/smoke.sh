...
if [[ -n "${EC2_APP_INSTANCE_ID}" ]]; then
  echo "EC2 app host smoke checks passed for ${APPLICATION_URL}."
  exit 0
fi
...
if [[ -n "${API_SERVICE_IMAGE_URI:-}" && -n "${USER_SERVICE_IMAGE_URI:-}" && -n "${AUTHORISATION_SERVICE_IMAGE_URI:-}" && -n "${ACADEMY_SERVICE_IMAGE_URI:-}" && -n "${ORGANISATION_SERVICE_IMAGE_URI:-}" && -n "${COURSE_SERVICE_IMAGE_URI:-}" && -n "${BOOKING_SERVICE_IMAGE_URI:-}" && -n "${PAYMENT_SERVICE_IMAGE_URI:-}" && -n "${SUBSCRIPTION_SERVICE_IMAGE_URI:-}" && -n "${ACCESS_KEY_SERVICE_IMAGE_URI:-}" && ( "${DISABLE_ANALYTICS_SERVICE:-false}" == "true" || -n "${ANALYTICS_SERVICE_IMAGE_URI:-}" ) && -n "${NOTIFICATION_SERVICE_IMAGE_URI:-}" && -n "${WALLET_SERVICE_IMAGE_URI:-}" && -n "${TRANSFER_SERVICE_IMAGE_URI:-}" && -n "${PRICING_SERVICE_IMAGE_URI:-}" ]]; then
  smoke_ports="8080 8081 8082 8083 8084 8085 8086 8087 8090 8091 8093 8094 8095 8096"
  if [[ "${DISABLE_ANALYTICS_SERVICE:-false}" != "true" ]]; then
    smoke_ports="${smoke_ports} 8092"
  fi
  PRIVATE_SMOKE_TASK_ARN="$(aws ecs run-task \
    ...
    --overrides "{\"containerOverrides\":[{\"name\":\"web\",\"command\":[\"sh\",\"-lc\",\"sleep 15; for port in ${smoke_ports}; do curl -fsS http://127.0.0.1:\\${port}/healthz; curl -fsS http://127.0.0.1:\\${port}/readyz || true; done\"]}]}" \
