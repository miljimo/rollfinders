...
function promotion_write() {
  ...
  ENV_FOR_PROMOTION="${env}" STATUS_FOR_PROMOTION="${status}" IMAGE_FOR_PROMOTION="${image_uri}" COMMIT_FOR_PROMOTION="${commit}" BRANCH_FOR_PROMOTION="${branch}" python3 -c '
import json, os, time
print(json.dumps({
...
promotion_require() {
  ...
  if [[ "${status}" != "success" || -z "${image}" ]]; then
    cat "${file}"
    rm -f "${file}"
    echo "${target_env} promotion is blocked because ${source_env} does not have a successful deploy record with an image."
    return 1
  fi

  echo "IMAGE_URI=${image}" >"${PROJECT_DIR:-$(pwd)}/image.env"
  rm -f "${file}"
  echo "${target_env} will promote image from ${source_env}: ${image}"
}
